...
from .models import GroupCourseTimes, Session
from django.db.models import Q


@receiver(pre_save, sender=GroupCourseTimes)
...
@receiver(post_save, sender=GroupCourseTimes)
def generate_sessions_for_group_courses_by_times(sender, instance, created, **kwargs):
    if created:
        ...
        this_group_course_sessions = Session.objects.filter(course= instance.course)
        ...
        for session in this_group_course_sessions:
            count = this_group_course_sessions.filter(
                Q(date__lt=session.date) | Q(Q(date=session.date) & Q(time_slot__start__lt=session.time_slot.start))
            ).count()
