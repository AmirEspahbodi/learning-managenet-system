...
from accounts.authtoken.serializers import TokenSerializer
...
class UserRegisterL2APIView(GenericAPIView):
    ...
    def post(self, *args, **kwargs):
        user_info = UserExtra.objects.get(user_id=self.request.data.get("user"))
        serializer = self.serializer_class(data=self.request.data, instance=user_info)
        serializer.is_valid(raise_exception=True)
        user_extra = serializer.save()
        return Response(
            {
                "user_id": user_extra.user.id,
                "national_code": user_extra.national_code,
                "secondary_phone_number": str(user_extra.secondary_phone_number),
                "province": user_extra.province,
                "city": user_extra.city,
                "address": user_extra.address,
                "postal_code": user_extra.postal_code,
                "date_of_birth": user_extra.date_of_birth,
                "father_name": user_extra.father_name,
                "home_phone_number": user_extra.home_phone_number,
            },
            status=status.HTTP_201_CREATED,
        )
